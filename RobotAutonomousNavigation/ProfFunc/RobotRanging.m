...
function [xR, yR] = RobotRanging(robotState, map, mScale)
...
xOrg = robotState(1)*mScale+szAug; yOrg = robotState(2)*mScale+szAug;

theta = robotState(3); rotMInv = [cos(theta), sin(theta); -sin(theta), cos(theta)];
...
for angIdx = 1:length(angRange)
    ...
    if (~isempty(idxMin))        
        ...
        ptRange(:,angIdx) = rotMInv*([xTmp;yTmp]-[xOrg;yOrg])/mScale + random('norm',0, 0.05, 2,1);
    end
end
